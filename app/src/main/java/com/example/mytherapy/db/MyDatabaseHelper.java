package com.example.mytherapy.db;
...
public class MyDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String DATABASE_NAME = "MyTherapy";
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        String query = String.format("CREATE TABLE IF NOT EXISTS %s (" +
                ...
                "%s INTEGER NOT NULL, " +
                "%s TEXT NOT NULL, " +
                "%s TEXT NOT NULL);", TABLE_NAME, KEY_ID, KEY_NAME, KEY_DAY, KEY_MONTH, KEY_YEAR, KEY_TIMES_PER_DAY, KEY_TOTAL_DOSES, KEY_TIMINGS, KEY_ALERT_TYPE);
        sqLiteDatabase.execSQL(query);
    }

    @Override
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int i, int i1) {
        String queryUpgrade = String.format("DELETE TABLE IF EXISTS %s", TABLE_NAME);
        sqLiteDatabase.execSQL(queryUpgrade);
        onCreate(sqLiteDatabase);
    }
    public void addNewMedicine(String medicineName, int day, int month, int year, int noOfTimesPerDay, int totalDoses, String timings, String alertType) {
    ...
    public List<String> getTimings(String medicineName) {
        ...
        Cursor cursor = db.query(TABLE_NAME, new String[]{KEY_TIMINGS}, KEY_NAME + " = ?", new String[]{medicineName}, null, null, null);
    ...
    public List<User> getMedicineList() {
        List<User> historyList = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.query(TABLE_NAME, new String[]{KEY_NAME, KEY_DAY, KEY_MONTH, KEY_YEAR, KEY_TIMES_PER_DAY, KEY_TOTAL_DOSES, KEY_TIMINGS}, null, null, null, null, null);
        while (cursor.moveToNext()) {
        ...
        User mainItem= new User(cursor.getString(0)  , date, cursor.getInt(4), cursor.getInt(5), cursor.getString(6));
            historyList.add(mainItem);
        }
        cursor.close();
        db.close();
        return historyList;
    ...
    public int noOfDaysLeft(String medicineName, Calendar mNextAlarmDate) {
        ...
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.query(TABLE_NAME, new String[]{KEY_DAY, KEY_MONTH, KEY_YEAR, KEY_TIMES_PER_DAY, KEY_TOTAL_DOSES}, KEY_NAME + " = ?", new String[]{medicineName}, null, null, null);
        while (cursor.moveToNext()) {
            day = cursor.getInt(0);
